...
// =============================================================================
// ApiError - Frozen API Error Type
// =============================================================================

/// API Error type (FROZEN)
///
/// This enum defines the public API error codes. These error codes and their
/// wire format are frozen and cannot change without a major version bump.
///
...
pub enum ApiError {
    ...
    HistoryTrimmed {
        ...
        requested: Version,
        /// The earliest version still available
...
impl ApiError {
    ...
    ///
    /// These strings are part of the wire protocol and cannot change.
    ...
    pub fn constraint_violation(reason: &str, extra_details: Option<crate::Value>) -> Self {
    ...
    /// Get error-specific details for wire encoding
    pub fn error_details(&self) -> Option<crate::Value> {
        use crate::Value;
        ...
        match self {
            ...
            ApiError::HistoryTrimmed {
                ...
                details.insert(
                    ...
                    version_to_value(earliest_retained),
                );
            }
            ApiError::Overflow => {
...
/// Convert a Version to a Value for wire encoding
fn version_to_value(v: &Version) -> crate::Value {
    use crate::Value;
    use std::collections::HashMap;

    let mut map = HashMap::new();
    let (type_name, value) = match v {
        Version::TxnId(n) => ("txn", *n),
        Version::Sequence(n) => ("sequence", *n),
        Version::Counter(n) => ("counter", *n),
    };
    map.insert("type".to_string(), Value::String(type_name.to_string()));
    map.insert("value".to_string(), Value::Int(value as i64));
    Value::Object(map)
}
...
pub struct WireError {
    /// Error code (matches ApiError variant name)
    pub code: String,
    /// Human-readable message
    pub message: String,
    /// Error-specific details (may be None)
...
pub mod constraint_reasons {
    ...
    /// Object/array nesting exceeds maximum depth
...
mod api_error_tests {
    ...
    fn test_all_error_codes_exist() {
        ...
        let codes = vec![
            ...
            ApiError::HistoryTrimmed {
                requested: Version::TxnId(1),
                earliest_retained: Version::TxnId(100),
            },
            ApiError::Overflow,
            ApiError::Internal {
                message: "bug".into(),
        ...
        let error_codes: HashSet<_> = codes.iter().map(|e| e.error_code()).collect();
    ...
    fn test_error_code_strings() {
        assert_eq!(
            ApiError::NotFound { key: "k".into() }.error_code(),
            "NotFound"
        );
        assert_eq!(
            ApiError::WrongType {
        ...
        assert_eq!(ApiError::Overflow.error_code(), "Overflow");
    }
    ...
    fn test_constraint_value_too_large() {
        let err = ApiError::constraint_violation("value_too_large", None);
        let wire = err.to_wire_error();
        ...
        match wire.details {
            Some(Value::Object(map)) => {
                assert_eq!(
                    map.get("reason"),
                    Some(&Value::String("value_too_large".into()))
                );
            }
    ...
    fn test_constraint_nesting_too_deep() {
        ...
        extra.insert("depth".to_string(), Value::Int(129));
        extra.insert("max".to_string(), Value::Int(128));

        let err = ApiError::constraint_violation("nesting_too_deep", Some(Value::Object(extra)));
        ...
        match wire.details {
            Some(Value::Object(map)) => {
                assert_eq!(
                    map.get("reason"),
                    Some(&Value::String("nesting_too_deep".into()))
                );
    ...
    fn test_constraint_key_too_long() {
        let err = ApiError::constraint_violation("key_too_long", None);
    ...
    fn test_all_constraint_reasons() {
        ...
        let reasons = vec![
            constraint_reasons::VALUE_TOO_LARGE,
            constraint_reasons::NESTING_TOO_DEEP,
            constraint_reasons::KEY_TOO_LONG,
            constraint_reasons::VECTOR_DIM_EXCEEDED,
            constraint_reasons::VECTOR_DIM_MISMATCH,
            constraint_reasons::ROOT_NOT_OBJECT,
            constraint_reasons::RESERVED_PREFIX,
            constraint_reasons::ARRAY_TOO_LONG,
            constraint_reasons::OBJECT_TOO_MANY_ENTRIES,
        ];

        for reason in reasons {
            let err = ApiError::constraint_violation(reason, None);
            let wire = err.to_wire_error();
    ...
    fn test_overflow_minimal_details() {
        ...
        assert!(
            wire.details.is_none()
                || matches!(wire.details, Some(Value::Object(ref m)) if m.is_empty())
        );
    }

    #[test]
    fn test_run_not_found_details() {
        ...
        match wire.details {
            Some(Value::Object(map)) => {
                assert_eq!(map.get("run_id"), Some(&Value::String("my-run".into())));
            }
            _ => panic!("Expected details with run_id"),
        }
    }

    #[test]
    fn test_run_closed_details() {
        let err = ApiError::RunClosed {
            run_id: "my-run".into(),
        };
        let wire = err.to_wire_error();
    ...
    #[test]
    fn test_run_exists_details() {
        let err = ApiError::RunExists {
            run_id: "my-run".into(),
        };
        let wire = err.to_wire_error();
    ...
    #[test]
    fn test_invalid_path_details() {
        let err = ApiError::InvalidPath {
            path: "$.invalid[".into(),
        ...
        match wire.details {
            Some(Value::Object(map)) => {
                assert_eq!(map.get("path"), Some(&Value::String("$.invalid[".into())));
                assert_eq!(
                    map.get("reason"),
                    Some(&Value::String("unclosed bracket".into()))
                );
            }
            _ => panic!("Expected details with path and reason"),
        }
    }

    #[test]
    fn test_internal_error_details() {
        ...
        match wire.details {
            Some(Value::Object(map)) => {
                assert_eq!(
                    map.get("message"),
                    Some(&Value::String("something went wrong".into()))
                );
            }
            _ => panic!("Expected details with message"),
        }
    }
}
