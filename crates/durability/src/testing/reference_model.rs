...
//! let mismatches = model.compare_kv("branch1", actual_kv_pairs);
...
pub struct ReferenceModel {
    /// KV state per branch: branch_name -> (key -> value)
    kv_state: HashMap<String, HashMap<String, Vec<u8>>>,
    /// Event state per branch: branch_name -> events
    event_state: HashMap<String, Vec<Vec<u8>>>,
    /// State values per branch: branch_name -> (key -> value)
...
impl ReferenceModel {
    ...
    pub fn kv_put(&mut self, branch: &str, key: &str, value: Vec<u8>) {
        self.kv_state
    ...
    pub fn kv_delete(&mut self, branch: &str, key: &str) {
        if let Some(branch_state) = self.kv_state.get_mut(branch) {
            branch_state.remove(key);
        }
    ...
    pub fn event_append(&mut self, branch: &str, payload: Vec<u8>) {
        self.event_state
            .entry(branch.to_string())
            .or_default()
            .push(payload.clone());
    ...
    pub fn state_set(&mut self, branch: &str, key: &str, value: Vec<u8>) {
        self.state_values
            .entry(branch.to_string())
            .or_default()
            .insert(key.to_string(), value.clone());
    ...
    /// Get expected KV value for a branch/key
    pub fn get_kv(&self, branch: &str, key: &str) -> Option<&Vec<u8>> {
        self.kv_state.get(branch)?.get(key)
    }

    /// Get all expected KV pairs for a branch
    pub fn get_kv_all(&self, branch: &str) -> Option<&HashMap<String, Vec<u8>>> {
        self.kv_state.get(branch)
    }

    /// Get expected events for a branch
    pub fn get_events(&self, branch: &str) -> Option<&Vec<Vec<u8>>> {
        self.event_state.get(branch)
    }

    /// Get expected state value for a branch/key
    pub fn get_state(&self, branch: &str, key: &str) -> Option<&Vec<u8>> {
        self.state_values.get(branch)?.get(key)
    }

    /// Get all branch names in the model
    ...
    pub fn compare_events(&self, branch: &str, actual: &[Vec<u8>]) -> Vec<StateMismatch> {
        let mut mismatches = Vec::new();
        ...
        if expected.len() != actual.len() {
            mismatches.push(StateMismatch {
                entity: format!("events:{}:count", branch),
    ...
    pub fn compare_state(
        ...
        let expected = self.state_values.get(branch);
        ...
        if let Some(expected_state) = expected {
            for (key, expected_value) in expected_state {
                match actual.get(key) {
                    ...
                    Some(actual_value) => {
                        mismatches.push(StateMismatch {
                            entity: format!("state:{}:{}", branch, key),
                            expected: format!("{:?}", expected_value),
                            actual: format!("{:?}", actual_value),
                        });
                    }
...
mod tests {
    ...
    fn test_kv_put() {
        ...
        assert_eq!(model.get_kv("branch1", "key1"), Some(&b"value1".to_vec()));
    ...
    fn test_kv_delete() {
        let mut model = ReferenceModel::new();
        model.kv_put("branch1", "key1", b"value1".to_vec());
        model.kv_delete("branch1", "key1");

        assert!(model.get_kv("branch1", "key1").is_none());
    ...
    fn test_event_append() {
        let mut model = ReferenceModel::new();
        model.event_append("branch1", b"event1".to_vec());
        model.event_append("branch1", b"event2".to_vec());

        let events = model.get_events("branch1").unwrap();
    ...
    fn test_compare_kv_match() {
        ...
        let mismatches = model.compare_kv("branch1", &actual);
        assert!(mismatches.is_empty());
    }
    ...
    fn test_compare_kv_missing() {
        ...
        assert_eq!(mismatches[0].entity, "kv:branch1:key2");
    }
    ...
    fn test_compare_events_match() {
        let mut model = ReferenceModel::new();
        model.event_append("branch1", b"e1".to_vec());
        model.event_append("branch1", b"e2".to_vec());

        let actual = vec![b"e1".to_vec(), b"e2".to_vec()];
        let mismatches = model.compare_events("branch1", &actual);
        assert!(mismatches.is_empty());
    }
    ...
    fn test_operations_list() {
        let mut model = ReferenceModel::new();
        model.kv_put("branch1", "key1", b"value1".to_vec());
        model.event_append("branch1", b"event".to_vec());
        model.checkpoint();
