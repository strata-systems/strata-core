...
fn test_kv_vector_all_persisted() {
...

    {
        ...
        let kv = test_db.kv();
        kv.put(&run_id, "counter", strata_core::value::Value::Int(1)).unwrap();
...
fn test_all_primitives_complex_workflow() {
...

    {
        ...
        for i in 0..5 {
            ...
            kv.put(&run_id, &format!("user:{}:login_count", i), strata_core::value::Value::Int(0)).unwrap();
...
fn test_all_primitives_partial_update() {
...

    {
        ...
        // Initial state
        kv.put(&run_id, "version", strata_core::value::Value::Int(1)).unwrap();
        vector.insert(run_id, "embeddings", "main", &seeded_random_vector(384, 1), None).unwrap();

        // Update both
        kv.put(&run_id, "version", strata_core::value::Value::Int(2)).unwrap();
    ...
    if let Some(versioned) = kv.get(&run_id, "version").unwrap() {
        if let strata_core::value::Value::Int(v) = versioned.value {
            assert_eq!(v, 2);
        }
    }
}
