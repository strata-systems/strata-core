...
fn test_facade_overhead_ratio() {
    ...
    // Warm up
    for i in 0..100 {
        kv.put(&run_id, &format!("warmup_{}", i), strata_core::value::Value::Int(i))
            .expect("put");
    }
    ...
    for i in 0..iterations {
        kv.put(&run_id, &format!("perf_key_{}", i), strata_core::value::Value::Int(i))
            .expect("put");
    }
...
fn test_non_transactional_fast_path() {
    ...
    kv.put(&test_db.run_id, "fast", strata_core::value::Value::Int(1))
        .expect("put");
}
