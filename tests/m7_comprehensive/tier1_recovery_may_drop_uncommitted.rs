...
fn test_r6_uncommitted_does_not_affect_committed() {
    ...
    kv.put(&run_id, "committed", Value::String("safe".into())).unwrap();

    // Second: more data (may be uncommitted at crash time)
    kv.put(&run_id, "maybe_uncommitted", Value::String("risky".into())).unwrap();
...
fn test_r6_partial_transaction_fully_discarded() {
    ...
    for key in &keys {
        kv.put(&run_id, key, Value::String("tx_value".into())).unwrap();
    }
...
fn test_r6_can_write_after_recovery() {
    ...
    let kv = test_db.kv();
    kv.put(&run_id, "before_crash", Value::String("value".into())).unwrap();

    test_db.reopen();

    // Should be able to write after recovery
    let kv = test_db.kv();
    kv.put(&run_id, "after_crash", Value::String("new_value".into())).unwrap();
...
fn test_r6_new_writes_after_loss() {
    ...
    let kv = test_db.kv();
    kv.put(&run_id, "original", Value::String("original_value".into())).unwrap();

    test_db.reopen();

    // Overwrite with new value
    let kv = test_db.kv();
    kv.put(&run_id, "original", Value::String("new_value".into())).unwrap();
