...
fn test_patch_ordering_matters() {
    // Scenario 1: Set ancestor then descendant
    {
        ...
        store.set(&run_id, &doc_id, &path("a"), JsonValue::object()).unwrap();
        store.set(&run_id, &doc_id, &path("a.b"), JsonValue::from(1i64)).unwrap();
...
fn test_last_write_wins_same_path() {
    ...
    for i in 1..=10 {
        store.set(&run_id, &doc_id, &path("value"), JsonValue::from(i as i64)).unwrap();
    }

    // Final value should be 10
    let result = store.get(&run_id, &doc_id, &path("value")).unwrap().unwrap();
...
fn test_overlapping_patches_conflict() {
    ...
    assert!(p1.conflicts_with(&p2), "Ancestor-descendant patches should conflict");
...
fn test_disjoint_patches_no_conflict() {
    ...
    assert!(!p1.conflicts_with(&p2), "Sibling patches should not conflict");
}
...
fn test_delete_set_same_subtree_conflict() {
    ...
    assert!(delete.conflicts_with(&set), "Delete ancestor should conflict with set descendant");
}
...
fn test_root_patch_conflicts_with_all() {
    ...
    assert!(root_patch.conflicts_with(&any_patch), "Root patch conflicts with all");
}
...
fn test_patches_see_prior_effects() {
    ...
    for i in 1..=5 {
        // Read current
        let current = store.get(&run_id, &doc_id, &path("counter"))
            .unwrap().unwrap().as_i64().unwrap();

        // Write incremented (simulating patch seeing prior state)
        store.set(&run_id, &doc_id, &path("counter"), JsonValue::from(current + 1)).unwrap();
    }

    let final_val = store.get(&run_id, &doc_id, &path("counter")).unwrap().unwrap();
...
fn test_intermediate_objects_visible_to_subsequent_ops() {
    ...
    store.set(&run_id, &doc_id, &path("a.b.c"), JsonValue::from(1i64)).unwrap();

    // Subsequent set on sibling path should work
    store.set(&run_id, &doc_id, &path("a.b.d"), JsonValue::from(2i64)).unwrap();

    // Both should exist
    let c = store.get(&run_id, &doc_id, &path("a.b.c")).unwrap().unwrap();
    let d = store.get(&run_id, &doc_id, &path("a.b.d")).unwrap().unwrap();
...
fn test_delete_removes_value() {
    ...
    store.delete_at_path(&run_id, &doc_id, &path("remove")).unwrap();

    assert!(store.get(&run_id, &doc_id, &path("remove")).unwrap().is_none());
    assert_eq!(
        store.get(&run_id, &doc_id, &path("keep")).unwrap().unwrap().as_str(),
        Some("this")
    );
}
...
fn test_delete_parent_removes_children() {
    ...
    // Delete parent
    store.delete_at_path(&run_id, &doc_id, &path("parent")).unwrap();

    // All children should be gone
    assert!(store.get(&run_id, &doc_id, &path("parent")).unwrap().is_none());
    assert!(store.get(&run_id, &doc_id, &path("parent.child1")).unwrap().is_none());
    assert!(store.get(&run_id, &doc_id, &path("parent.nested.deep")).unwrap().is_none());
}
...
fn test_set_creates_value() {
    ...
    store.set(&run_id, &doc_id, &path("new.path"), JsonValue::from("created")).unwrap();

    let val = store.get(&run_id, &doc_id, &path("new.path")).unwrap().unwrap();
...
fn test_set_overwrites_value() {
    ...
    store.set(&run_id, &doc_id, &path("field"), JsonValue::from("updated")).unwrap();

    let val = store.get(&run_id, &doc_id, &path("field")).unwrap().unwrap();
...
fn test_set_root_replaces_all() {
    ...
    store.set(&run_id, &doc_id, &root(), serde_json::json!({"new": "doc"}).into()).unwrap();

    // Old data gone
    assert!(store.get(&run_id, &doc_id, &path("old")).unwrap().is_none());
    assert!(store.get(&run_id, &doc_id, &path("lots")).unwrap().is_none());
...
fn test_set_preserves_siblings() {
    ...
    store.set(&run_id, &doc_id, &path("parent.modify"), JsonValue::from("changed")).unwrap();

    // Siblings preserved
    assert_eq!(
        store.get(&run_id, &doc_id, &path("parent.sibling1")).unwrap().unwrap().as_str(),
        Some("keep")
    );
    assert_eq!(
        store.get(&run_id, &doc_id, &path("parent.sibling2")).unwrap().unwrap().as_str(),
        Some("keep")
    );
    assert_eq!(
        store.get(&run_id, &doc_id, &path("parent.modify")).unwrap().unwrap().as_str(),
        Some("changed")
    );
}
...
fn test_version_is_document_scoped() {
    ...
    store.set(&run_id, &doc_id, &path("a"), JsonValue::from(100i64)).unwrap();
    let v2 = store.get_version(&run_id, &doc_id).unwrap().unwrap();

    store.set(&run_id, &doc_id, &path("b"), JsonValue::from(200i64)).unwrap();
