...
fn test_set_not_commutative_same_path() {
    ...
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(1i64)).unwrap();
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(2i64)).unwrap();
    let result1 = store1.get(&run_id1, &doc_id1, &path("x")).unwrap().unwrap().as_i64();

    // Order 2: set x=2, then x=1
    let (_, store2, run_id2, doc_id2) = setup_doc(JsonValue::object());
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(2i64)).unwrap();
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(1i64)).unwrap();
    let result2 = store2.get(&run_id2, &doc_id2, &path("x")).unwrap().unwrap().as_i64();
...
fn test_set_delete_not_commutative() {
    ...
    let (_, store1, run_id1, doc_id1) = setup_doc(JsonValue::object());
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(1i64)).unwrap();
    store1.delete_at_path(&run_id1, &doc_id1, &path("x")).unwrap();
    ...
    store2.delete_at_path(&run_id2, &doc_id2, &path("x")).unwrap();
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(1i64)).unwrap();
...
fn test_different_paths_commutative() {
    ...
    let (_, store1, run_id1, doc_id1) = setup_doc(JsonValue::object());
    store1.set(&run_id1, &doc_id1, &path("a"), JsonValue::from(1i64)).unwrap();
    store1.set(&run_id1, &doc_id1, &path("b"), JsonValue::from(2i64)).unwrap();
    let a1 = store1.get(&run_id1, &doc_id1, &path("a")).unwrap().unwrap().as_i64();
    let b1 = store1.get(&run_id1, &doc_id1, &path("b")).unwrap().unwrap().as_i64();

    // Order 2: set b=2, then a=1
    let (_, store2, run_id2, doc_id2) = setup_doc(JsonValue::object());
    store2.set(&run_id2, &doc_id2, &path("b"), JsonValue::from(2i64)).unwrap();
    store2.set(&run_id2, &doc_id2, &path("a"), JsonValue::from(1i64)).unwrap();
    let a2 = store2.get(&run_id2, &doc_id2, &path("a")).unwrap().unwrap().as_i64();
    let b2 = store2.get(&run_id2, &doc_id2, &path("b")).unwrap().unwrap().as_i64();
...
fn test_set_sequence_order_matters() {
    ...
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(1i64)).unwrap();
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(2i64)).unwrap();
    store1.set(&run_id1, &doc_id1, &path("x"), JsonValue::from(3i64)).unwrap();
    let result1 = store1.get(&run_id1, &doc_id1, &path("x")).unwrap().unwrap().as_i64();
    ...
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(1i64)).unwrap();
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(2i64)).unwrap();
    store2.set(&run_id2, &doc_id2, &path("x"), JsonValue::from(3i64)).unwrap();
    let result2 = store2.get(&run_id2, &doc_id2, &path("x")).unwrap().unwrap().as_i64();
...
fn test_set_overwrites_completely() {
    ...
    store.set(&run_id, &doc_id, &path("data"), serde_json::json!({"c": 3}).into()).unwrap();

    // Old keys gone, only new key exists
    assert!(store.get(&run_id, &doc_id, &path("data.a")).unwrap().is_none());
    assert!(store.get(&run_id, &doc_id, &path("data.b")).unwrap().is_none());
    assert_eq!(store.get(&run_id, &doc_id, &path("data.c")).unwrap().unwrap().as_i64(), Some(3));
}
...
fn test_set_nested_no_sibling_effect() {
    ...
    // Set specific path
    store.set(&run_id, &doc_id, &path("data.a"), JsonValue::from(100i64)).unwrap();

    // Sibling unchanged
    assert_eq!(store.get(&run_id, &doc_id, &path("data.a")).unwrap().unwrap().as_i64(), Some(100));
    assert_eq!(store.get(&run_id, &doc_id, &path("data.b")).unwrap().unwrap().as_i64(), Some(2));
}
...
fn test_type_change_on_set() {
    ...
    // x is integer
    assert!(store.get(&run_id, &doc_id, &path("x")).unwrap().unwrap().is_i64());

    // Set x to string
    store.set(&run_id, &doc_id, &path("x"), JsonValue::from("hello")).unwrap();
...
fn test_scalar_to_object() {
    ...
    // Set x to object
    store.set(&run_id, &doc_id, &path("x"), serde_json::json!({"nested": true}).into()).unwrap();

    // Can access nested field
    assert_eq!(
        store.get(&run_id, &doc_id, &path("x.nested")).unwrap().unwrap().as_bool(),
        Some(true)
    );
}
...
fn test_object_to_scalar() {
    ...
    assert_eq!(store.get(&run_id, &doc_id, &path("x")).unwrap().unwrap().as_i64(), Some(42));
    assert!(store.get(&run_id, &doc_id, &path("x.nested")).unwrap().is_none());
}
...
fn test_complex_operation_sequence() {
    ...
    store.set(&run_id, &doc_id, &path("users"), serde_json::json!([{"name": "Alice"}]).into()).unwrap();
    store.set(&run_id, &doc_id, &path("users[0].age"), JsonValue::from(30i64)).unwrap();
    store.set(&run_id, &doc_id, &path("config"), serde_json::json!({"enabled": true}).into()).unwrap();
    store.delete_at_path(&run_id, &doc_id, &path("config.enabled")).unwrap();
    store.set(&run_id, &doc_id, &path("config.debug"), JsonValue::from(false)).unwrap();
    ...
    assert!(store.get(&run_id, &doc_id, &path("config.enabled")).unwrap().is_none());
    assert_eq!(
        store.get(&run_id, &doc_id, &path("config.debug")).unwrap().unwrap().as_bool(),
        Some(false)
    );
}
