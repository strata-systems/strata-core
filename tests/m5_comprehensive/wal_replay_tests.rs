...
fn test_wal_replay_deterministic() {
    ...
    for _ in 0..5 {
        ...
        store.set(&run_id, &doc_id, &path("d"), JsonValue::from(4i64)).unwrap();

        // Verify final state
        assert_eq!(store.get(&run_id, &doc_id, &path("a")).unwrap().unwrap().as_i64(), Some(1));
        assert!(store.get(&run_id, &doc_id, &path("b")).unwrap().is_none());
        assert_eq!(store.get(&run_id, &doc_id, &path("c")).unwrap().unwrap().as_i64(), Some(3));
        assert_eq!(store.get(&run_id, &doc_id, &path("d")).unwrap().unwrap().as_i64(), Some(4));
...
fn test_wal_replay_order_matters() {
    // Two different orderings produce different results
    {
        ...
        store.set(&run_id, &doc_id, &path("x"), JsonValue::from(2i64)).unwrap();
...
fn test_operations_are_atomic() {
    ...
    store.set(&run_id, &doc_id, &path("complex"), complex_value).unwrap();

    // Either the whole complex value is there, or none of it
    let nested_value = store.get(&run_id, &doc_id, &path("complex.nested.deep.value")).unwrap();
...
fn test_multi_document_interleaved_operations() {
    ...
    store.set(&run_id, &doc1, &root(), JsonValue::from(1i64)).unwrap();
    store.set(&run_id, &doc2, &root(), JsonValue::from(2i64)).unwrap();
    store.set(&run_id, &doc1, &root(), JsonValue::from(11i64)).unwrap();
    store.set(&run_id, &doc3, &root(), JsonValue::from(3i64)).unwrap();
    store.set(&run_id, &doc2, &root(), JsonValue::from(22i64)).unwrap();
    store.set(&run_id, &doc3, &root(), JsonValue::from(33i64)).unwrap();

    // Each document has correct final state
    assert_eq!(store.get(&run_id, &doc1, &root()).unwrap().unwrap().as_i64(), Some(11));
    assert_eq!(store.get(&run_id, &doc2, &root()).unwrap().unwrap().as_i64(), Some(22));
    assert_eq!(store.get(&run_id, &doc3, &root()).unwrap().unwrap().as_i64(), Some(33));
}
...
fn test_complex_nested_operations() {
    ...
    store.set(&run_id, &doc_id, &path("users"), JsonValue::array()).unwrap();
    ...
    store.set(&run_id, &doc_id, &path("users"), serde_json::json!([user1.clone()]).into()).unwrap();

    // Modify nested value
    store.set(&run_id, &doc_id, &path("users[0].settings.theme"), JsonValue::from("light")).unwrap();

    // Delete a field
    store.delete_at_path(&run_id, &doc_id, &path("users[0].email")).unwrap();

    // Verify final state
    let theme = store.get(&run_id, &doc_id, &path("users[0].settings.theme")).unwrap().unwrap();
    assert_eq!(theme.as_str(), Some("light"));

    let email = store.get(&run_id, &doc_id, &path("users[0].email")).unwrap();
    assert!(email.is_none());

    let name = store.get(&run_id, &doc_id, &path("users[0].name")).unwrap().unwrap();
...
fn test_long_operation_sequence() {
    ...
    for i in 0..100 {
        let key = format!("key_{}", i);
        let val = store.get(&run_id, &doc_id, &key.parse().unwrap()).unwrap().unwrap();
...
fn test_alternating_set_delete() {
    ...
    for i in 0..10 {
        // Set
        store.set(&run_id, &doc_id, &path("temp"), JsonValue::from(i as i64)).unwrap();
        assert_eq!(
            store.get(&run_id, &doc_id, &path("temp")).unwrap().unwrap().as_i64(),
            Some(i as i64)
        );

        // Delete
        store.delete_at_path(&run_id, &doc_id, &path("temp")).unwrap();
        assert!(store.get(&run_id, &doc_id, &path("temp")).unwrap().is_none());
    }
...
fn test_run_isolation_in_wal() {
    ...
    // run2 unaffected
    assert_eq!(store.get(&run1, &doc_id, &root()).unwrap().unwrap().as_i64(), Some(100));
    assert_eq!(store.get(&run2, &doc_id, &root()).unwrap().unwrap().as_i64(), Some(2));

    // Modify run2
    store.set(&run2, &doc_id, &root(), JsonValue::from(200i64)).unwrap();

    // run1 unaffected
    assert_eq!(store.get(&run1, &doc_id, &root()).unwrap().unwrap().as_i64(), Some(100));
    assert_eq!(store.get(&run2, &doc_id, &root()).unwrap().unwrap().as_i64(), Some(200));
}
...
fn test_durability_mode_semantic_equivalence() {
    ...
    test_across_modes("json_create_and_modify", |db| {
        ...
        let b_val = store.get(&run_id, &doc_id, &path("b")).unwrap().unwrap().as_i64();
...
fn test_empty_document_operations() {
    ...
    assert!(store.get(&run_id, &doc_id, &path("anything")).unwrap().is_none());

    // Delete from empty object (idempotent)
    store.delete_at_path(&run_id, &doc_id, &path("nonexistent")).unwrap();
...
fn test_null_value_operations() {
    ...
    store.set(&run_id, &doc_id, &root(), JsonValue::object()).unwrap();

    // Now we can set fields
    store.set(&run_id, &doc_id, &path("field"), JsonValue::from(1i64)).unwrap();
    assert_eq!(
        store.get(&run_id, &doc_id, &path("field")).unwrap().unwrap().as_i64(),
        Some(1)
    );
}
...
fn test_array_document_operations() {
    ...
    // Read element
    assert_eq!(store.get(&run_id, &doc_id, &path("[1]")).unwrap().unwrap().as_i64(), Some(2));

    // Modify element
    store.set(&run_id, &doc_id, &path("[1]"), JsonValue::from(99i64)).unwrap();
    assert_eq!(store.get(&run_id, &doc_id, &path("[1]")).unwrap().unwrap().as_i64(), Some(99));

    // Delete element (shifts remaining)
    store.delete_at_path(&run_id, &doc_id, &path("[0]")).unwrap();

    // [1] is now what was [2]
    assert_eq!(store.get(&run_id, &doc_id, &path("[0]")).unwrap().unwrap().as_i64(), Some(99));
    assert_eq!(store.get(&run_id, &doc_id, &path("[1]")).unwrap().unwrap().as_i64(), Some(3));
}
