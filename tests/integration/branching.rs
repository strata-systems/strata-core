...
fn child_branch_does_not_inherit_parent_data_currently() {
    ...
    let child_value = kv.get(&child_branch_id, "parent_key").unwrap();
...
#[test]
#[ignore = "Waiting for issue #780 - BranchCreateChild should copy parent data"]
fn child_branch_should_inherit_parent_data() {
    let test_db = TestDb::new();
    let branch_index = test_db.branch_index();
    let p = test_db.all_primitives();

    // Create parent branch
    let parent_meta = branch_index.create_branch("fork_parent").unwrap();
    ...
    // Fork (create child with parent) - using create_branch since create_branch_with_options doesn't exist
    let child_meta = branch_index.create_branch("fork_child").unwrap();
